...
import { Booking, ClassItem, Course, AttendanceRecord } from '../types';
import { COURSES } from '../data/content';
import { RAZORPAY_KEY_ID } from '../lib/config';
import { loadRazorpayScript } from '../lib/razorpay';
import { ClassHomeworkSection } from './StudentHomeworkSection';
...
export const StudentPortal: React.FC<StudentPortalProps> = ({
  ...
  const calculateStudentProgress = (courseName: string) => {
    const classesList = courseClasses[courseName] || [];
    ...
    const attendedClasses = classesList.filter((cls) => {
      const rec = attendanceRecords.find((r) => r.class_id === cls.id);
  ...
  useEffect(() => {
    ...
    // Realtime subscription for instant booking & attendance updates
                    ...
                    {isDemo ? (
                      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-blue-100 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
                          {(() => {
                            const demoStatus = booking.demo_attendance_status || 'Pending';
                            let badgeStyle = 'bg-slate-100 text-slate-700 border-slate-200';
                            if (demoStatus === 'Attended') {
                              badgeStyle = 'bg-emerald-100 text-emerald-800 border-emerald-300 font-black';
                            } else if (demoStatus === 'Absent') {
                              badgeStyle = 'bg-red-100 text-red-800 border-red-300 font-bold';
                            }
                            return (
                              <div className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs shadow-2xs ${badgeStyle}`}>
                                <UserCheck className="w-4 h-4 shrink-0" />
                                <span>Attendance: {demoStatus}</span>
                              </div>
                            );
                          })()}
                          <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-green-50 border border-green-200 text-green-800 text-xs font-black">
                      ...
                      <div className="space-y-4">
                        ...
                        {(() => {
                          const prog = calculateStudentProgress(booking.course_name);
                          return (
                            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-blue-100 shadow-2xs space-y-3">
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                <div className="flex items-center gap-2">
                                  <UserCheck className="w-4 h-4 text-[#1E40AF]" />
                                  <h4 className="text-xs sm:text-sm font-extrabold text-slate-800">
                                    Live Class Attendance & Completion Progress
                                  </h4>
                                </div>

                                <div className="flex items-center gap-2 flex-wrap">
                                  {prog.isCompleted && (
                                    <span className="px-3 py-1 bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-full text-xs font-black flex items-center gap-1.5 shadow-2xs">
                                      <Award className="w-4 h-4 text-emerald-600" />
                                      Course Completed
                                    </span>
                                  )}
                                  <span className="text-xs font-extrabold text-[#1E40AF] bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100">
                                    {prog.label}
                                  </span>
                                </div>
                              </div>
                              ...
                              {prog.totalClasses > 0 ? (
                                <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden border border-slate-200">
                                  <div
                                    className="bg-gradient-to-r from-[#1E40AF] via-blue-600 to-emerald-500 h-full rounded-full transition-all duration-500"
                                    style={{ width: `${prog.percentage}%` }}
                                  />
                                </div>
                              ) : (
                                <p className="text-xs text-slate-400 font-medium italic">
                                  No class sessions scheduled yet for this course.
                                </p>
                              )}

                              {prog.isCompleted && (
                                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-xs font-bold flex items-center gap-2 mt-2">
                                  <Award className="w-4 h-4 text-emerald-600 shrink-0" />
                                  <span>
                                    Congratulations! You have completed 100% of the live sessions in this course!
                                  </span>
                                </div>
                              )}
                            </div>
                        ...
                        <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-2 pt-2">
                        ...
                        {isLoadingCls ? (
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {classesList.map((cls) => {
                              const rec = attendanceRecords.find((r) => r.class_id === cls.id);
                              const statusVal = rec?.status || 'Pending';
                              ...
                              return (
                                <div
                                  key={cls.id}
                                  className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-2xs hover:border-blue-300 transition-all flex flex-col gap-3"
                                >
                                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="space-y-2 flex-1">
                                      <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border ${badgeStyle}`}>
                                          {statusVal}
                                        </span>
                                  ...
                                  {/* Per-Class Homework Upload & List */}
                                  {user && (
